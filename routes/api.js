...
route.put("/product/:id", (req, res) => {
  let item = req.body;
  let sql =
    "update product set product_name = ?, product_description = ?, rate = ? where product_id = ?";
  db.query(
    sql,
    [item.name, item.description, item.rate, req.params.id],
    (err, row) => {
...
route.put("/customer/:id", (req, res) => {
  ...
  let user = req.body;
  let sql =
    "update customer set customer_name = ?, address = ?, phone = ?, email = ? where customer_id = ?";
  db.query(
    sql,
    [user.name, user.address, user.phone, user.email, id],
    (err, row) => {
      if (err) throw err;
...
route.post("/customer", (req, res) => {
  ...
  db.query(
    sql,
    [0, user.name, user.address, user.phone, user.email],
    (err, row) => {
      if (err) throw err;
...
route.get("/transaction", (req, res) => {
  let sql =
    "select transaction_id, c.customer_id, c.customer_name, p.product_name, p.rate, quantity, (p.rate * quantity) as Total from salestransaction natural join customer as c natural join product as p";
  db.query(sql, (err, row) => {
    if (err) throw err;
    res.send(row);
  });
...
route.get("/transaction/:id", (req, res) => {
  let t = req.body;
  let sql =
    "select c.customer_id, p.product_id, c.customer_name, p.product_name, p.rate, quantity, (p.rate * quantity) as Total from salestransaction natural join customer as c natural join product as p where transaction_id = ?";
  db.query(sql, [req.params.id], (err, row) => {
    if (err) throw err;
    res.send(row);
  });
...
route.put("/transaction/:id", (req, res) => {
  let t = req.body;
  let sql =
    "update salestransaction set customer_id = ?, product_id = ?, quantity = ? where transaction_id = ? ";
  db.query(
    sql,
    [t.customer_id, t.product_id, t.quantity, req.params.id],
    (err, row) => {
      if (err) throw err;
...
route.get("/generate-invoice/:id", (req, res) => {
  let sql =
    "select  c.customer_id, c.customer_name,p.product_name, p.product_description, p.rate, quantity, (p.rate * quantity) as Total from salestransaction natural join customer as c natural join product as p where customer_id = ?";
  db.query(sql, [req.params.id], (err, row) => {
    if (err) throw err;
    res.send(row);
  });
